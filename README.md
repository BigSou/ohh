...
public class Chromosome {
    ...
    public static ArrayList<Chromosome> selector(ArrayList<Chromosome> fatherGroup,int sonGroupSize) {
        ...
        int index = 0;
        for(Chromosome chrom : fatherGroup) {//计算适应度
            fitness[index] = chrom.calcFitness() / ((double)totalFitness);
            index++;
        }
        for(int i = 1; i < fitness.length; i++) {//计算累加适应度
            fitness[i] = fitness[i-1]+fitness[i];
        }
        for(int i = 0; i < sonGroupSize; i++) { //轮盘赌选择
            Random random = new Random();
    ...
    public static void main(String[] args) throws IOException {
        ...
        Chromosome theBest;
        BufferedWriter bw = new BufferedWriter(new FileWriter("C:\\Users\\90837\\Desktop\\aa.txt"));
        int i = 0;
        do{
            ...
            bw.write(String.valueOf(theBest.calcFitness()));
            bw.newLine();
            i++;
        }while(i<100);
        bw.close();
